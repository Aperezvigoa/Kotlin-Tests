...
class Library {
    ...
    fun findBookByTitle() {
        ...
        println("$coincidences found.")
    }

    fun findBookByAuthor() {
        print("Write the name of the author: ")
        val inputAuthor = readln()
        var coincidences = 0
        for(book in books) {
            if (inputAuthor == book.author) {
                println(book.printInfo())
                println("---------------------------------")
                coincidences++
            }
...
fun main() {
    ...
    println("******************************")
    do {
        println("1. Register a book")
        println("2. Register a partner")
        println("3. Lend a book")
        println("4. Return a book")
        println("5. Find book by title")
        println("6. Find book by author")
        println("7. Exit")
        println("Please select a option.")
        input = readln().toInt()
        when (input) {
            1 -> libreria.bookRegister()
            2 -> libreria.partnerRegister()
            3 -> libreria.lendABook()
            4 -> libreria.returnABook()
            5 -> libreria.findBookByTitle()
            6 -> libreria.findBookByTitle()
            7 -> break
        }
        println("\n\n")
    } while (input != 7)
